                           ...
                           esl_sensor_sensor_code_t code)
{
  ...
  if (state == esl_state_boot) {
    const uint8_t remaining = (uint8_t)(&sensor_info[ESL_SENSOR_INFO_POOL_SIZE]
                                        - (uint8_t *)sensor_registry.next_object
                                        ->info);
    ...
    if (remaining > info_size) {
      uint8_t *current_info_ptr =
        (uint8_t *)sensor_object[sensor_registry.sensor_count].info;

      if (is_custom) {
        sensor_registry.next_object->info->size =
          ESL_SENSOR_SIZE_VENDOR_SPECIFIC;
      ...
      // setup next object
      sensor_object[sensor_registry.sensor_count].info =
        (esl_sensor_info_p)current_info_ptr;
...
sl_status_t esl_sensor_read(uint8_t index)
{
  ...
  if (index < sensor_registry.sensor_count) {
    ...
    esl_sensor_sensor_code_t code = (size == ESL_SENSOR_SIZE_MESH_DEVICE)
                                    ? sensor_object[index].info->
                                    mesh_property_id
                                    : sensor_object[index].info->vendor_specific
                                    .sensor_code;
    esl_sensor_company_id_t company = (size == ESL_SENSOR_SIZE_MESH_DEVICE)
                                      ? ESL_SENSOR_MESH_DEVICE
                                      : sensor_object[index].info->
                                      vendor_specific.company_id;
    ...
    if ((result != SL_STATUS_OK) && (company == ESL_SENSOR_MESH_DEVICE)) {
      ...
      switch (code) {
        ...
        case PRESENT_DEVICE_OPERATING_TEMPERATURE: {
          temperature_t temp =
            (temperature_t)((EMU_TemperatureGet() + .5f) * 100.f);
