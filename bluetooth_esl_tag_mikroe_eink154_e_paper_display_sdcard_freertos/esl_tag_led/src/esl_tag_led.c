                                   ...
                                   void *data)
{
  ...
  // check repeat type
  if ((led->control.repeats & ESL_LED_REPEATS_TYPE_MASK)
      == ESL_LED_REPEATS_TYPE_COUNT) {
    esl_led_repeats_type_t repeat_count;

    repeat_count =
      (led->control.repeats
       & (esl_led_repeats_type_t)(~ESL_LED_REPEATS_TYPE_MASK))
    ...
    if ((repeat_count != 0)
        && (esl_core_get_status() != esl_state_unassociated)) {
      esl_led_control_t *control = &led->control;

      lit = esl_led_get_pattern_state(control, led->pattern_position);
      time =
        lit ? control->pattern.bit_on_period : control->pattern.bit_off_period;
      time = time << 1;
    } else {
    ...
    if (status == SL_STATUS_OK) {
      ...
      uint32_t          duration = led->control.repeats >>
                                   ESL_LED_REPEATS_DURATION_SHIFT;
                        ...
                        uint8_t blue_value)
{
  ...
  if (state == esl_state_boot) {
    ...
    if (led_registry.led_count < ESL_TAG_MAX_LEDS) {
      ...
      new_led |= (red_value & ESL_LED_GENERIC_2BIT_MASK) <<
                 ESL_LED_RED_GAMUT_SHIFT;
      new_led |= (green_value & ESL_LED_GENERIC_2BIT_MASK) <<
                 ESL_LED_GREEN_GAMUT_SHIFT;
      new_led |= (blue_value & ESL_LED_GENERIC_2BIT_MASK) <<
