...
#include "sl_gpio.h"
#include "sl_spidrv_sdcard_config.h"
...
static void sdcard_init(void)
{
  ...
  const sl_gpio_t miso_gpio = {
    .port = SL_SPIDRV_SDCARD_RX_PORT,
    .pin = SL_SPIDRV_SDCARD_RX_PIN
  };
  sl_gpio_set_pin_mode(&miso_gpio,
                       SL_GPIO_MODE_INPUT_PULL,
                       true);
  sd_card_spi_init(sl_spidrv_sdcard_handle);

  // Give a work area to the default drive
  FRESULT ret_code = f_mount(&fs, "", 0);
  app_assert(ret_code == FR_OK);
}
