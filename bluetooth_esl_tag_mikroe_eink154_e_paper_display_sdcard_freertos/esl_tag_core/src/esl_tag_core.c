...
#define ESL_PAWR_INTEGER_INTERVAL         (uint32_t)( \
...
static void esl_core_secondary_advertiser(void *p_event_data,
                                          uint16_t event_size)
{
  (void)event_size;
  (void)p_event_data;
  ...
  (void)sl_bt_legacy_advertiser_start(esl_tag_persistent.advertising_set_handle[
                                        1],
...
{
  sl_status_t sc;
  uint32_t pawr_interval =
    sl_sleeptimer_tick_to_ms(esl_tag.pawr_interval_ticks);
  uint16_t scan_interval = pawr_interval
                           / (ESL_DRIFT_INTEGER_MULTIPLIER
                              * ESL_TAG_SCAN_SWEEEP_COEFF);
  uint16_t scan_window = scan_interval
                         > ESL_TAG_SCAN_WINDOW_MS ? ESL_TAG_SCAN_WINDOW_MS
                         :scan_interval;
  ...
  if (sc == SL_STATUS_OK) {
    sl_bt_esl_log(ESL_LOG_COMPONENT_CORE,
                  ...
                  scan_window,
                  ESL_TAG_SCAN_TIMEOUT_SEC);
    sc = sl_bt_scanner_start(sl_bt_gap_phy_1m,
...
void esl_core_step(void)
{
  if (esl_tag_persistent.watchdog_triggered) {
    ...
    } else if (ESL_TAG_POWER_DOWN_ENABLE
               && (esl_state_unassociated == esl_tag.status)) {
...
static void esl_state_boot_handler(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    case sl_bt_evt_system_boot_id: {
      ...
      sc = sl_bt_advertiser_create_set(
        &esl_tag_persistent.advertising_set_handle[0]);
      sl_bt_esl_assert(sc == SL_STATUS_OK);

#if ESL_TAG_INTERMITTENT_ADVERTISING
      // create secondary advertising set for speeding up connection requests
      sc = sl_bt_advertiser_create_set(
        &esl_tag_persistent.advertising_set_handle[1]);
      sl_bt_esl_assert(sc == SL_STATUS_OK);
#endif // ESL_TAG_INTERMITTENT_ADVERTISING
...
static void esl_state_connectable_handler(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    case sl_bt_evt_sm_bonding_failed_id:
      ...
      if (evt->data.evt_sm_bonding_failed.reason
      ...
      } else if (evt->data.evt_sm_bonding_failed.reason
        ...
        if (memcmp(&esl_tag.ap_address.addr, ESL_INVALID_AP_ADDRESS,
                   sizeof(bd_addr)) != 0) {
          sl_bt_esl_log(ESL_LOG_COMPONENT_CORE,
                        ESL_LOG_LEVEL_WARNING,
    ...
    case sl_bt_evt_connection_opened_id:
      ...
      if (evt->data.evt_connection_opened.bonding == esl_tag.bonding_handle) {
        // store AP address
        memcpy(&esl_tag.ap_address.addr,
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
        ...
        switch (evt->data.evt_gatt_server_user_write_request.characteristic) {
          case gattdb_esl_address:
            if (evt->data.evt_gatt_server_user_write_request.value.len
                == sizeof(esl_address_t)) {
              esl_address_t address =
                *(esl_address_t *)evt->data.evt_gatt_server_user_write_request.
                value
                .data;
              // do not allow the broadcast address to be set - neither by mistake!
              if (esl_core_get_id(address) != ESL_BROADCAST_ID) {
                // ignore RFU bit
                esl_tag.address.value = address
                                        & (ESL_GROUP_ID_MASK | ESL_ID_MASK);
          ...
          case gattdb_esl_response_key:
            ...
            if (((evt->data.evt_gatt_server_user_write_request.att_opcode
              ...
              if (overall_size == SL_BT_EAD_KEY_MATERIAL_SIZE) {
                ...
                (void)app_scheduler_add(&esl_core_async_randomizer,
                                        NULL,
                                        0,
                                        NULL);
                if (sc == SL_STATUS_OK) {
              ...
              result = SL_STATUS_BT_ATT_INVALID_ATT_LENGTH;
              memset((void *)&esl_keys.response_key,
                     0,
          ...
          case gattdb_esl_absolute_time:
            if (evt->data.evt_gatt_server_user_write_request.value.len
              ...
              CORE_ATOMIC_SECTION(
                ...
                esl_tag.time_offset -= sl_sleeptimer_tick_to_ms(
    ...
    case sl_bt_evt_gatt_server_characteristic_status_id:
      if ((evt->data.evt_gatt_server_characteristic_status.characteristic
        ...
        switch (evt->data.evt_gatt_server_characteristic_status.
                client_config_flags) {
    ...
    case sl_bt_evt_periodic_sync_transfer_received_id:
      ...
      (void)sl_bt_connection_close(
        evt->data.evt_periodic_sync_transfer_received.connection);
      sl_bt_esl_log(ESL_LOG_COMPONENT_CORE,
                    ESL_LOG_LEVEL_WARNING,
    ...
    case sl_bt_evt_pawr_sync_transfer_received_id:
      ...
      (void)sl_bt_connection_close(
        evt->data.evt_pawr_sync_transfer_received.connection);
      ...
      if (evt->data.evt_pawr_sync_transfer_received.status == SL_STATUS_OK) {
        ...
        sync_timeout =
          (10 * evt->data.evt_pawr_sync_transfer_received.adv_interval) / 8;
        // accept the sync transfer only from the bonded AP according to ESL spec.

        if (evt->data.evt_pawr_sync_transfer_received.bonding
            != esl_tag.bonding_handle) {
    ...
    case sl_bt_evt_connection_closed_id:
      ...
      if (!is_esl_configured_for(ESL_CONFIG_FLAGS_MANDATORY_MASK)) {
        ...
        if ((esl_tag.sync_handle != SL_BT_INVALID_SYNC_HANDLE)
          ...
          (void)sl_sleeptimer_restart_periodic_timer_ms(
            &esl_tag_persistent.watchdog_handle,
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_pawr_sync_subevent_report_id:
      ...
      if (evt->data.evt_pawr_sync_subevent_report.data_status == 0) {
        ...
        if (msg && (msg[0] == --len) && (msg[1] == ESL_AD_TYPE)) {
          ...
          esl_tag_persistent.request_event =
            evt->data.evt_pawr_sync_subevent_report.event_counter;
          esl_tag_persistent.request_subevent =
          ...
          if (esl_tag.status == esl_state_synchronized) {
            esl_group_id_t group_id = esl_core_get_group_id(
              esl_tag.address.value);
            ...
            if (*(esl_group_id_t *)&msg[ESL_PAYLOAD_OVERHEAD] == group_id) {
              ...
              static uint32_t last_measured =
                (UINT32_MAX - ESL_TAG_BATTERY_MEASUREMENT_INTERVAL_MS);
              uint32_t now = sl_sleeptimer_tick_to_ms(
                sl_sleeptimer_get_tick_count());

              // only measure if the requested interval has passed (saves power)
              if ((uint32_t)(now - last_measured)
        ...
        if (interval_count != 0) {
          ...
          sl_bt_esl_log(ESL_LOG_COMPONENT_CORE,
                        ...
                        (int16_t)(esl_tag.tick_error
                                  / (int32_t)ESL_DRIFT_THRESHOLD_DIVISOR),
                        (uint16_t)abs((esl_tag.tick_error
                                       % (int32_t)ESL_DRIFT_THRESHOLD_DIVISOR)),
                        error_threshold);
        }
        ...
        if (sl_sleeptimer_tick64_to_ms(tick_now,
                                       &ticks_elapsed) == SL_STATUS_OK) {
    ...
    case sl_bt_evt_scanner_extended_advertisement_report_id:
      ...
      if (esl_tag.sync_handle == SL_BT_INVALID_SYNC_HANDLE) {
        ...
        if (evt->data.evt_scanner_extended_advertisement_report.bonding
            == esl_tag.bonding_handle) {
          ...
          uint32_t pawr_interval =
            ...
            / 8;
          uint32_t sync_timeout =
            ((ESL_TAG_MAX_SYNC_LOST_COUNT * pawr_interval)
             + ESL_SYNC_MIN_TIMEOUT)
            / 10;
...
sl_status_t esl_core_start_advertising(void)
{
  ...
  esl_state_t new_esl_status = esl_tag.bonding_handle
  ...
  if (esl_tag.connection_handle == SL_BT_INVALID_CONNECTION_HANDLE) {
    ...
    result = sl_bt_advertiser_set_timing(
      esl_tag_persistent.advertising_set_handle[0],
      16 * min_interval / 10,
      // configurator asks users for [ms],
      16 * max_interval / 10,
      // but our API expects in 0.625ms units
      0,
      // adv. duration
      0);                                    // max. num. adv. events
#if ESL_TAG_INTERMITTENT_ADVERTISING
    // set the secondary advertiser with the same timing parameters as the primary one
    (void)sl_bt_advertiser_set_timing(esl_tag_persistent.advertising_set_handle[
                                        1],
                                      16 * min_interval / 10,
                                      16 * max_interval / 10,
                                      0,
    ...
    if (result == SL_STATUS_OK) {
      ...
      result = sl_bt_legacy_advertiser_start(
        esl_tag_persistent.advertising_set_handle[0],
        sl_bt_legacy_advertiser_connectable);
      sl_bt_esl_log(ESL_LOG_COMPONENT_CORE,
                    ESL_LOG_LEVEL_INFO,
    ...
    if ((esl_tag.status != esl_state_unassociated)
      ...
      if (ESL_TAG_POWER_DOWN_ENABLE
          && (esl_tag.status == esl_state_unassociated)) {
...
sl_status_t esl_core_get_randomizer(sl_bt_ead_randomizer_t randomizer)
{
  ...
  if (!!(esl_tag.config_status & ESL_CONFIG_FLAG_RANDOMIZER_READY)) {
    ...
    if (result == SL_STATUS_OK) {
      memcpy((void *)esl_core_response_randomizer.randomizer,
             (void *)nonce.randomizer,
