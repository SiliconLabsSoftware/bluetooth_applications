/***************************************************************************//**
 * @file app.c
 * @brief CS Based PC Locking example application logic
 *******************************************************************************
 * # License
...
static uint8_t cs_advertising_set_handle = 0xff;
static uint8_t reflector_connection_id = 0xff;
static uint8_t laptop_pc_connection_id = 0xff;
...
SL_WEAK void app_init(void)
{
  ...
  app_log("Channel Sounding Based PC Locking\r\n");

  sc = cs_based_pc_locking_init();
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_connection_opened_id:
      if (evt->data.evt_connection_opened.role
        ...
        // Increasing security connection when connected with Laptop/PC as HID keyboard
        laptop_pc_connection_id = evt->data.evt_connection_opened.connection;
        sc = sl_bt_sm_increase_security(
          evt->data.evt_connection_opened.connection);
        app_assert_status(sc);
      } else {
        // When the opened connection with the cs_reflector device,
        // enabled start advertising to the Laptop/PC as HID keyboard.
        reflector_connection_id = evt->data.evt_connection_opened.connection;

        // Create an advertising set.
        sc = sl_bt_advertiser_create_set(&cs_advertising_set_handle);
        app_assert_status(sc);

        // Generate data for advertising
        sc = sl_bt_legacy_advertiser_generate_data(
          cs_advertising_set_handle,
        ...
        sc = sl_bt_legacy_advertiser_start(cs_advertising_set_handle,
